...
(function($, window, undefined) {
	console.log("call -> Cazary -> inicio");
	// console.log($);
	// console.log(window);
	...
	var _ = (function() {
		/**
		 * Translation Data see i18n directory
		 */
	...
	var checkEmail = (function() {
		...
		return function(string) {
			...
			if (string.indexOf("@") > 64) {
				// length of local part is up to 64 charactersï¼ˆcf. RFC 5321
				// 4.5.3.1.1.ï¼‰
				return false;
			}
	...
	var EditorCore = (function() {
		var STATUS_NORMAL = 0, STATUS_ACTIVE = 1, STATUS_DISABLED = 2;

		var COMMAND_FONTNAME = "fontname", COMMAND_FONTSIZE = "fontsize", COMMAND_BOLD = "bold", COMMAND_ITALIC = "italic", COMMAND_UNDERLINE = "underline", 
		COMMAND_STRIKETHROUGH = "strikethrough", COMMAND_REMOVEFORMAT = "removeformat", COMMAND_FORECOLOR = "forecolor", COMMAND_BACKCOLOR = "backcolor", 
		COMMAND_HILITECOLOR = "hilitecolor", COMMAND_SUPERSCRIPT = "superscript", COMMAND_SUBSCRIPT = "subscript", COMMAND_JUSTIFYLEFT = "justifyleft", 
		COMMAND_JUSTIFYCENTER = "justifycenter", COMMAND_JUSTIFYRIGHT = "justifyright", COMMAND_JUSTIFYFULL = "justifyfull", COMMAND_INDENT = "indent", 
		COMMAND_OUTDENT = "outdent", COMMAND_ORDEREDLIST = "insertorderedlist", COMMAND_UNORDEREDLIST = "insertunorderedlist", 
		COMMAND_INSERTHORIZONTALRULE = "inserthorizontalrule", COMMAND_INSERTIMAGE = "insertimage", COMMAND_CREATELINK = "createlink", 
		COMMAND_UNLINK = "unlink", COMMAND_UNDO = "undo", COMMAND_REDO = "redo", COMMAND_PARAMETRO = "parametro";
		...
		return function(edit, value, style) {
			...
			this.COMMAND_PARAMETRO = COMMAND_PARAMETRO;
			...
			this.setPasteHtmlAtCaret = _pasteHtmlAtCaret;
			...
			function _construct(value) {
				 ...
				 * type="text/css">[STYLE]</style> </head> <body></body>
				 * </html>
				 */
				var iframehtml = '<!DOCTYPE html><html><head><meta charset="UTF-8" /><style type="text/css">'
			...
			function _execCommand(commandName, parameters) {
				console.log("call -> execCommand");
				console.log(commandName);
				console.log(parameters);
				// if browser supports "hilitecolor", use it.
				if (commandName == COMMAND_BACKCOLOR
						&& _canExecCommand(COMMAND_HILITECOLOR)) {
			...
			function _getValue() {
				...
				// replace tags
				html = html.replace(/(<\/?)p\b/gi, "$1div").replace(
						/(<\/?)em\b/gi, "$1i").replace(/(<\/?)strong\b/gi,
						"$1b").replace(/(<\/?)del\b/gi, "$1s").replace(
						/^<br\s*\/?>$/i, "");
				return html;
			}
			...
			function _getCurrentStatus() {
				...
				$.each([ COMMAND_CREATELINK, COMMAND_UNLINK, COMMAND_UNDO,
						COMMAND_REDO ], function(index, value) {
			...
			function _getCurrentStyle() {
				...
				return node.currentStyle
						|| contentDocument.defaultView.getComputedStyle(
								node.parentElement, "");
			}
			...
			function _pasteHtmlAtCaret(html, selectPastedContent) {
				console.log("call -> pasteHtmlAtCaret");
				console.log(contentWindow.getSelection);
				...
				if (contentWindow.getSelection) {
					...
					if (sel.getRangeAt && sel.rangeCount) {
						...
						// Range.createContextualFragment() would be useful here
						// but is
	...
	var wCazary = $.fn
			.extend({
				cazary : (function($) {
					// keycodes
					var KEYCODE_ENTER = 13, KEYCODE_ESCAPE = 27;

					/*
					 * <div class="cazary"> <!-- commands wrapper is here -->
					 * <iframe class="cazary-edit" src="javascript:"></iframe>
					 * <!-- original textarea is here --> </div>
					 */
					...
					var PRE_DEFINED_MACROS = {
						...
						"bold italic underline strikethrough",
								"createlink unlink" ],
						"FULL" : [
								"parametro fontname fontsize",
								"bold italic underline strikethrough removeformat | forecolor backcolor | superscript subscript",
								"justifyleft justifycenter justifyright justifyfull | indent outdent | insertorderedlist insertunorderedlist",
								// "inserthorizontalrule insertimage createlink
								// unlink",
								"inserthorizontalrule createlink unlink",
								"undo redo", "source" ]
					...
					return function(options) {
						...
						return this
								.each(function() {
									...
									$cazary
											.prepend(createCommandsWrapper(options.commands));

									// editor object
									var $cazary_edit = $cazary.find(
											".cazary-edit").css({
										height : $origin.height()
									});

									// source command & others
									var $cazary_command_source = $cazary
											.find("ul.cazary-commands-list li.cazary-command-source");
									var $cazary_commands_except_source = $cazary
											.find("ul.cazary-commands-list li:not(.cazary-command-source)");

									// set objects
									$origin.hide().before($cazary).insertAfter(
											$cazary_edit).addClass(
											"cazary-source");

									var editor = new EditorCore($cazary_edit
											.get(0), $origin.val(),
											options.style);

									var commands_generic = [
											editor.COMMAND_PARAMETRO,
											editor.COMMAND_BOLD,
											editor.COMMAND_ITALIC,
											editor.COMMAND_UNDERLINE,
											editor.COMMAND_STRIKETHROUGH,
											editor.COMMAND_REMOVEFORMAT,
											editor.COMMAND_SUPERSCRIPT,
											editor.COMMAND_SUBSCRIPT,
											editor.COMMAND_JUSTIFYLEFT,
											editor.COMMAND_JUSTIFYCENTER,
											editor.COMMAND_JUSTIFYRIGHT,
											editor.COMMAND_JUSTIFYFULL,
											editor.COMMAND_INDENT,
											editor.COMMAND_OUTDENT,
											editor.COMMAND_ORDEREDLIST,
											editor.COMMAND_UNORDEREDLIST,
											editor.COMMAND_INSERTHORIZONTALRULE,
											editor.COMMAND_UNLINK,
											editor.COMMAND_UNDO,
											editor.COMMAND_REDO ];
									var commands_with_panel = [
											editor.COMMAND_FONTNAME,
											editor.COMMAND_FONTSIZE,
											editor.COMMAND_FORECOLOR,
											editor.COMMAND_BACKCOLOR,
											editor.COMMAND_INSERTIMAGE,
											editor.COMMAND_CREATELINK ];
									...
									// editor events
									$(editor.contentDocument).on("select",
											function() {
									...
									$(editor.contentWindow).on("focus",
											function() {
												destroyAllPanels();
												_updateCommandStatus();
											}).on("blur", function() {
										// update original element when focus is
										// out
										$origin.val(editor.value());
									});

									// cancel other handler when command is
									// disabled
									$cazary
											.on(
													"click",
									...
									$cazary_command_source.on("click",
											function() {
												_toggleMode();
												_setFocus();
											});
									// command handler
									console.log("call -> commands_generic");
									console.log(commands_generic);
									$
											.each(
													commands_generic,
													function() {
																...
																.on(
																		...
																		function() {
																			console
																					.log($(this));
																			console
																					.log("call -> cazary-command -> click4");
																			// execute
																			// command
																			if (commandName == "parametro") {
																				visualizarModal();
																				// editor.setPasteHtmlAtCaret("<i>inserted</i>",false);
																				// _execCommand("insertText","<input
																				// type='text'
																				// value='teste'>");
																			} else
									...
									function _execCommand(commandName,
											parameters) {
										destroyAllPanels();
									...
									function visualizarModal() {
										var that = this;
										console
												.log("call -> CadastroTermoControle -> visualizarModal");
										var wModulo = $("#modulo").val();
										...
										$
												.when(
														$
																.get('../fes-web/servicos/administracao/cadastrotermo/controle/CadastroTermoModalControle.js'),
														$
																.get('../fes-web/servicos/administracao/cadastrotermo/modelo/CadastroTermoModal.js'))
												.done(
														function success() {
															console
																	.log("call -> CadastroTermoControle -> consultar");
															var wModel = new CadastroTermoModal();
															wModel.set(
																	"codigo",
																	wModulo);

															var wE = new CadastroTermoModalControle(
																	{
																		model : wModel
																	});

															wE.eventChange = function(
																	value) {

																value = '<input class="C5x" disabled="" value="'
																		+ value
																		+ '" @@="">';
																// console.log("value:
																// " + value );
																//editor.setPasteHtmlAtCaret("<b>teste</b>");
																editor.setPasteHtmlAtCaret(value,false);
																$origin.val(editor.value());
															};
									...
									function _setRteMode() {
										...
										$cazary_commands_except_source
												.removeClass("cazary-disabled");
										$cazary_command_source
												.removeClass("cazary-active");
										_updateCommandStatus();
									}
									...
									function _setHtmlMode() {
										...
										$cazary_commands_except_source
												.addClass("cazary-disabled");
										$cazary_command_source
												.addClass("cazary-active");
									}
									...
									function _isHtmlMode() {
										return $cazary_command_source
												.hasClass("cazary-active");
									}
									function _isRteMode() {
										return !_isHtmlMode();
									}
									 ...
									 */
									function createPanel(commandName, options,
											$command) {
										var $panel = $(".cazary-panel");
										if ($panel.length > 0) {
											var uniqueId_panel = $panel
													.data("id");
											var commandName_panel = $panel
													.data("command");
											destroyAllPanels();
											if (commandName_panel === commandName
													&& uniqueId_panel === uniqueId) {
												_setFocus();
												return;
											}
										}
										...
										case editor.COMMAND_BACKCOLOR:
											$panel = createPanel_color(
													commandName, options.colors);
											list = true;
											break;
										...
										if (list) {
											...
											$panel
													.on(
															...
															function() {
																console
																		.log("call -> cazary-command -> click2");
										...
										var offset = $command.addClass(
												"cazary-active").offset();
										offset.top += $command.outerHeight();
										$panel.addClass("cazary-panel")
												.addClass(
														"cazary-panel-"
																+ commandName)
												.data("id", uniqueId).data(
														"command", commandName)
												.css({
													left : offset.left + "px",
													top : offset.top + "px"
												...
												.find(":text:first").trigger(
														"focus");

										return $panel;
									}

									function createPanel_fontname(commandName,
											fontnames) {
										/*
										 * <div class="cazary-panel
										 * cazary-panel-fontname">
										 * <ul class="cazary-widget-select">
										 * <li unselectable="on" style="font-family: XXX" title="XXX" data-param="XXX">XXX</li>
										 * ... </ul> </div>
										 */
										var $ul = $("<ul />").addClass(
												"cazary-widget-select");
									...
									function createPanel_fontsize(commandName) {
										/*
										 * <div class="cazary-panel
										 * cazary-panel-fontsize">
										 * <ul class="cazary-widget-select">
										 * <li unselectable="on" title="Smallest" data-param="1"><font
										 * size="1">Size 1</font></li>
										 * <li unselectable="on" title="Smallest" data-param="2"><font
										 * size="2">Size 2</font></li>
										 * <li unselectable="on" title="Smallest" data-param="3"><font
										 * size="3">Size 3</font></li>
										 * <li unselectable="on" title="Smallest" data-param="4"><font
										 * size="4">Size 4</font></li>
										 * <li unselectable="on" title="Smallest" data-param="5"><font
										 * size="5">Size 5</font></li>
										 * <li unselectable="on" title="Smallest" data-param="6"><font
										 * size="6">Size 6</font></li>
										 * <li unselectable="on" title="Smallest" data-param="7"><font
										 * size="7">Size 7</font></li> </ul>
										 * </div>
										 */
										var $ul = $("<ul />").addClass(
												"cazary-widget-select");
										$.each(ASSOC_FONTSIZES, function(param,
												text) {
											var text = _(text);
											var $li = $("<li />").attr({
												"unselectable" : "on",
									...
									function createPanel_color(commandName,
											colors) {
										/*
										 * <div class="cazary-panel
										 * cazary-panel-[COMMANDNAME]">
									...
									function createPanel_insertimage(
											commandName) {
										/*
										 * <div class="cazary-panel
										 * cazary-panel-insertimage"> <form
										 * action="#"> <div> <fieldset>
										 * <legend>Input image URL</legend>
										 * <input type="url"
										 * class="cazary-widget-insertimage-url"
										 * required="required"
										 * placeholder="http://example.com/path/to/image.jpg" />
										 * </fieldset> <input type="button"
										 * class="cazary-widget-submit"
										 * value="Insert" /> </div> <fieldset
										 * class="cazary-widget-preview">
										 * <legend>Preview</legend> <img
										 * class="cazary-widget-preview-insertimage" />
										 * </fieldset> </form> </div>
										 */
										$panel = $("<div>")
												.append(
																		...
																		$(
																				...
																				.append(
																						$(
																								"<img />")
																								.addClass(
																										"cazary-widget-preview-insertimage"))));
										...
										function onsubmit() {
											console
													.log("call -> cazary-command -> click 5");

											var $url = $panel
													.find(".cazary-widget-insertimage-url");
											var url = $url.val();
											if (!checkURL(url)) {
												$url.trigger("focus");
												return false;
										...
										function onupdate() {
											console
													.log("call -> cazary-command -> click 6");
									...
									function createPanel_createlink(commandName) {
										/*
										 * <div class="cazary-panel
										 * cazary-panel-createlink"> <form
										 * action="#"> <div> <fieldset>
										 * <legend>Input link URL or E-mail
										 * address</legend> <input type="url"
										 * class="cazary-widget-createlink-url"
										 * required="required"
										 * placeholder="http://example.com/,
										 * someone@example.com" /> </fieldset>
										 * <input type="button"
										 * class="cazary-widget-submit"
										 * value="Insert" /> </div> <fieldset
										 * class="cazary-widget-preview">
										 * <legend>Preview</legend> <iframe
										 * class="cazary-widget-preview-createlink"></iframe>
										 * </fieldset> </form> </div>
										 */
										var $panel = $("<div>")
												.append(
																						...
																						$(
																								...
																								.append(
																										$(
																												"<legend />")
																												.text(
																														_("Input link URL or E-mail address")))
																								.append(
																										$(
																												"<input type=\"text\" />")
																												.addClass(
																														"cazary-widget-createlink-url")
																												.attr(
																														{
																															"required" : "required",
										...
										function onsubmit() {
											var $url = $panel
													.find(".cazary-widget-createlink-url");
											var url = $url.val();
											if (checkEmail(url)) {
												url = "mailto:" + url;
													...
													.setTimeout(
															function() {
																...
																if (checkURL(url)) {
																	...
																	$panel
																			.find(
																					".cazary-widget-preview-createlink")
																			.attr(
																					"src",
																					url);
																} else {
									...
									/**
									 * update command status when to be called:
									 * <ul>
									...
									function _updateCommandStatus() {
										...
										for ( var name in status) {
											...
											// set font color
											if (name === editor.COMMAND_FORECOLOR
													|| name === editor.COMMAND_BACKCOLOR) {
												var $command = $cazary
														.find(".cazary-command-"
																+ name);
												var color = value;
					...
					function createCommandsWrapper(commands) {
						 ...
						 * ... </ul> <ul class="cazary-commands-list">
						...
						var $obj = $("<div />").addClass(
								"cazary-commands-wrapper");
						$.each(commands, function() {
							var $ul = $("<ul />").addClass(
									"cazary-commands-list");
